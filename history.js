...
async function add_game_to_player_standing(game, uid, only_update_element) {
	...
	if (only_update_element) {
		winmix(only_update_element, percent)
		return
	}
...
function winmix(element, percent) {
...
async function get_tournament_details(tid, get_games) {
		...
		if (tournament.status == 'completed') {
			for (let box of document.querySelectorAll(`.box[data-kind="tournament"][data-id="${tid}"]`)) {
				...
				if (tournament.standing) {
					winmix(box, tournament.standing*100)
				}
			}
		} else {
			for (let box of document.querySelectorAll(`.box[data-kind="tournament"][data-id="${tid}"]`)) {
	...
	return {
		pid: tournament.my_pid, // relies on response_promise, sometimes
		players: players,  		// relies on response_promise
		games: games,  			// relies on games_promise
		active: active  		// relies on games_promise
	};
}
...
async function compare_players_from_game(id) {
	...
	await load_active_players_history(uids, game);
}
...
async function cache_all_tournaments() {
	let boxes = [...document.querySelectorAll('.box[data-kind="tournament"][data-id]:not(.cached)')]
	let tids = []
	...
	document.getElementById('caching').classList.remove('hide')
	document.getElementById('cache-button').classList.add('hide')
	await Promise.all(tids.map(async (tid) => {
		await get_tournament_details(tid, true)
	}))
	document.getElementById('cache-box').classList.add('hide')
	document.getElementById('caching').classList.add('hide')
	document.getElementById('cache-button').classList.remove('hide')
}
...
async function add_player_button(uid) {
...
missing = {}
function add_missing(kind, id) {
	if (!missing[kind]) missing[kind] = []
	missing[kind].push(id)
}
async function find_missing() {
	// console.log('finding missing')
	// console.log(missing)
	if (missing.arena) {
		let query = {
			arenas: missing.arena.slice(0, 25).join(',')
		}
		missing.arena = missing.arena.slice(25)
		let getter = get({
			endpoint: 'arenas',
			query: query
		}).catch(console.log)
		let objs = await getter
		console.log(query, objs)
		for (let obj of objs.data) {
			console.log(obj)
		}
	}
	if (missing.arena) {
		console.log('still missing some arenas')
		// setTimeout(find_missing, 0)
	}
}
async function name(kind, id) {
	let name;
	if (id === null) {
		if (kind == 'user') {
	...
	} else if (kind == 'user' && id == myUserId) {
		name = 'Me';
	} else if (all_data[kind][id]) {
		name = all_data[kind][id].name;
	} else if (kind == 'tournament' || kind == 'arena') {
...
function save_data(kind, obj) {
	...
	if (!all_data[kind][id]) {
		...
		if (obj.name) {
			for (node of document.querySelectorAll(`.${kind}-name[data-id="${id}"`)) {
...
async function game_element(game, inc_players, inc_tournament, won) {
	...
	if (inc_players) {
		...
		for (uid of game.userIds) {
			...
			li.append(rankdiv)
			li.append(await title('user', uid, 'span'));  // not actually async
			plist.append(li);
		}
	}
	if (inc_tournament) {
...
async function add_tournament(tournament, manual) {
	...
	if (cached_tourney) {
		...
		if (cached_tourney.standing) {
			winmix(box, cached_tourney.standing*100)
		}
	} else {
...
async function add_tournament_from_manual(tid) {
	...
	*/
	...
	await get_tournament_details(tid, false)
...
function tabselect(id) {
	...
	me.scrollIntoView({inline: 'center'})
